...
command = "/home/apps/kafka/bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic cha_stream --consumer-property group.id=sgcho |grep stream_queue_key | grep syncombine1  |grep create_order |grep 20230810-0002702 |head -n 5"
result = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
